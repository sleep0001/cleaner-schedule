    ...
    return {
      ...
      children: (
        <div className="custom-timeline-item">
          ...
          <div>
            {log.date1}: <strong>{log.peopleBefore.join(', ')}</strong> to <strong>{log.peopleAfter.join(', ')}</strong>
          </div>
          <div>
            {log.date2}: <strong>{log.peopleAfter.join(', ')}</strong> to <strong>{log.peopleBefore.join(', ')}</strong>
          </div>
        </div>
      )
    };
